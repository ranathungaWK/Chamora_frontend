...
export function TestCycleComparisonPage() {
  ...
  const canCompare = selectedRuns.length >= 2 && selectedRuns.length <= 4 && selectedMetrics.length > 0 && selectedMetrics.length <= 5;
          ...
          <div className="lg:col-span-2 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                ...
                <p className="mt-2 text-2xl font-bold text-emerald-700">{canCompare ? 'Yes' : 'No'}</p>
              </div>
            </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500">{run.date} · {run.duration}</p>
                            <p className="mt-2 text-sm text-slate-600">{run.avgResponseTime} avg response time · {run.throughput} throughput</p>
                            <p className="mt-1 text-sm text-slate-500">Error rate {run.errorRate} · Success rate {run.successRate}</p>
                          </div>
                        </div>
                  ...
                  return (
                    <div
                      ...
                      className={`cursor-pointer rounded-2xl border p-4 transition-all hover:-translate-y-0.5 hover:shadow-md ${
                        isSelected ? 'border-purple-400 bg-purple-50 shadow-sm' : 'border-slate-200 bg-white'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3">
                          <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                            isSelected ? 'bg-gradient-to-br from-purple-500 to-indigo-600' : 'bg-slate-100'
                          }`}>
                            <MetricIcon className={`w-5 h-5 ${isSelected ? 'text-white' : 'text-slate-500'}`} />
                          </div>
                          <div>
          ...
          <div className="space-y-6">
            <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-6 shadow-sm sticky top-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-slate-800">Chat preview</h3>
                  <p className="text-sm text-slate-600">Your comparison context will open in chat.</p>
                </div>
              </div>
                  ...
                  <div className="mt-2 space-y-2">
                    {selectedRuns.length > 0 ? (
                      selectedRuns.map((run) => (
                        <div key={run.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                          {run.name}
                        </div>
                      ))
                    ) : (
